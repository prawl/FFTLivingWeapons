...
14184F8AC at (7,4) vs twin 1418500AC). The ±1MB BandScan fallback was removed (the walk re-finds within
...
`BandReadBase = CombatAnchor+0x1C−24*0x200`, 49 slots, validity = sane bounds incl. brave/faith ≤100);
the static array remains the ENEMY-IDENTITY/team oracle (capture (lvl,br,fa,mhp) from enemy-side slots
each onField tick — **by sane fields, NOT inb: live RE shows the array's "inBattle" u16 PULSES 0/1 per
unit mid-battle** (half the live enemies read 0; gating on it refused their kills as "not a captured
enemy" — Patrick hit exactly this). Guards: 3-tick alive/dead streaks (identity-bound, onField-gated),
...
my first draft was rejected with 3 blockers — battleMode is a CURSOR-TILE-CLASS encoder, Paused reads 0,
...
2. **Polling death / no resets across battles**: slot9 dropped mid-battle and stuck across the world map.
   FIX: new pure `BattleState` machine (`LivingWeapon/BattleState.cs`, 24 tests) — instant enter
...
3. **Kill inflation guard**: ResetBattle → baseline poll marks pre-existing corpses settled (false/dup
...
action → KILL line ~1s; `battle:` edges between chained battles; turn counter resets; pause/dialogue >5s
mid-battle produce NO exit edge.

## Extra-turn: PARKED, fully diagnosed + a new design lever (Patrick's idea)
- v7 died of LOCATOR AMBIGUITY: every arm → `ambiguous locate (2 matches)` — two stride-aligned copies of
  ...
  stride match + PA/MA 1..199 sanity + per-battle cache — its stat writes provably land on the live copy).
- **Speed-lever alternative (Patrick, 2026-06-09)**: instead of slamming CT, hold the killer's SPEED
  (combat +0x40) at ≥100 for the grant window — CT refills +Speed per clock tick, so the gauge fills in
  ONE tick after the kill-turn ends. Every primitive is already shipping (GrowthEngine locates the struct,
  writes Speed today for Swiftfang growth, and HoldTimedStat has capture-natural→restore). Same queue-
...
test (and the old handoff's proposed "fresh read" fix) sat on a poisoned oracle — **the condensed
...
From the 12:44–12:55 live log of the v6 build:
- **Bug A confirmed**: all 13 grants stuck `Owed: active=True` → `window expired`. Never reached Pinning.
- **Bug B dissolved**: 12/13 kill credits landed with killer CT=100 → credit lands DURING the kill-turn;
  KillTracker was never ~15s late. The "delayed bonus" was Bug A parking CT at 100 → window expiry → the
  killer's natural queue slot arriving later. (1/13 credited post-turn at ct=5 — late credits exist, rare.)
- The located auth copy `@14184F8AC` = `CombatAnchor+0x1C` (CT = combat base+0x41 ✓); Ramza's struct never
...
- **Pull-down counting**: the engine pulls CT below ~70 in exactly one case — a turn of that unit ENDED.
  We read before each re-slam, so pull-downs are countable. Arm-time CT ≥100 (2 agreeing reads) ⇒ kill-turn
  running ⇒ expect 2 pull-downs (kill-turn end, bonus end); <100 ⇒ expect 1. Release on the last = consumed.
- **Locate by wielder**: killer = the roster slot holding the Zwill (`RRHand/RLHand/ROffHand == 10`); found
  per tick by stride walk `CombatAnchor ± 24*0x200` matching `CWeapon(+0x20)` ∈ wielder's hand ids AND
  brave/faith (+0x2A/+0x2C); exact-Zwill match outranks; ambiguity ⇒ no slam; throttled band-scan fallback.
- **Hardening**: window = NO-SIGNAL timeout (12s, refreshed while CT≥100 — a player deliberating through a
  long kill-turn can't expire the grant) + 90s absolute cap; **CT=0 restore on every non-consumed release**
  (kills the parked-100 ghost grant = the old Bug B mechanism); took needs a 3-read streak (refractory —
  turn-end oscillation can't double-count); killer HP==0 (3 reads) ⇒ release; Arming does NOT slam
  (classification reads stay unpolluted). States: `Idle → Arming → Owed → Pinning → Idle`.
- **Patrick's decisions**: extra-turn REPLACES Dual Wield (items.json id 10 now has a label-only signature
  `displayLabel: "Extra Turn"` — paints the card at P3, arms no support bit; 221 was build-time-only and
  never worked live anyway); NO chaining (kill mid-grant just logs); verbose events DEV-only.
- **`SlamCt = 105` is wired but UNVERIFIED** — only 100 is live-proven. First thing in the verify session:
  `%TEMP%\fft_probes\ct_probe.py hold combat 105 <mhp> <lvl>` — does the write stick, and does 105 schedule
...
- **PendingTtl 30 → 90**: the constant assumed the 100ms poll (~3s); at the 33ms tick it expired at ~1s.
- **BattleLog.cs** (dev builds): per-tick damage/heal/move event lines tagged with the latched weapons.
  ...
  milliseconds — on-screen action vs captured event is now comparable at tick granularity.
...
2. Zwill kills at normal speed, including one where you deliberate >12s after the kill before ending the
   turn. Expect in the log: `arm` → `classified -> Owed (kill-turn running, two pull-down(s) owed)` →
   `pull-down #1` → `pull-down #2` → `release (Consumed)`, and the bonus turn on screen right after the
   kill-turn ends. Late-credit kills classify Pinning and need one pull-down. Check no ghost turn after
   any `release (NoSignal)` — the CT restore should prevent it.
3. The dev `ev:` lines + `turn:` lines + ms timestamps are the latency oracle for the tracker hardening.

## After verify (in order)
1. Revert **Zwill WP 99 → 12** in `data/items.json` (proposed.wp), regenerate, gates green.
2. Commit (imperative mood, no attribution — pre-commit hook enforces).
...
- 112/112 tests, analyze gate PASS, deployed (DEV: LwDev=true — thresholds {1,2,3}, all weapons seeded P2,
  verbose events ON).
