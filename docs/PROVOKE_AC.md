...
Two halves, and the game supplies the better one.

**The rage is the engine's.** Provoke inflicts Berserk on the unit you point at. Berserk is the one
status in this game that is fully behavioural from the flag alone: the unit goes mindless, stops
using abilities, charges, and swings. That is not a marker dressed up as a taunt, it is the engine
genuinely enraging a unit for us.

**The tunnel vision is ours.** Berserk sends a unit at whoever is NEAREST, which on its own is a
coin flip: an enraged enemy standing beside your mage still mauls your mage. So while the shout
lasts, the runtime flags every other unit on your side with the engine's own "cannot be seen"
status, leaving the bearer as the only name on the target list. The engine supplies the rage; we
supply the direction.

The second half is subtractive because it has to be. There is no aggro in this game: a PSX dig
closed that question, enemy targeting is computed tile scoring with no holdable focus field, so the
only lever on who the AI attacks is whether a unit can be targeted at all.

Berserk also prices the ability honestly. It raises the provoked unit's physical attack while taking
its abilities away, so a provoked enemy hits the bearer HARDER than it otherwise would. Against the
game's best parry that is a good trade and a real decision, not a free win.
...
- **Re-effected** by repointing ONE byte: the action row's InflictStatus index, from 45 (DontMove)
  to 53 (Berserk alone). That byte lives at `+15` of the ability's row in the LIVE action table at
  `0x14078B2DC`, NOT the decoy copy at `0x14078961C`.

Because the ability marks its own target with a status, the runtime needs no cast-detection hook and
no action-record read: it polls for an enemy wearing Berserk, which is a read it already does.
...
0b. It targets a single enemy at range 5, costs no MP, has no charge time, and lands at 100% on a
   non-immune target.
0c. It inflicts Berserk and nothing else. The action row's InflictStatus index is held at 53 in the
   LIVE table (`0x14078B2DC`); the decoy copy is never written.
0d. Jobs immune to Berserk are a known gap. The ability will report its real chance on the targeting
   screen, so the player is never misled, but a Provoke that cannot land is a Provoke that does
   nothing. Measure the immune set before shipping and state it on the item card.
