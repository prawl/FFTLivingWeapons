...
STATUS: ARCHIVED (the 2.0 release shipped; current scope lives in docs/RELEASE_SCOPE.md)
...
- [x] **Equip-card kills counter + suffix paint** out of battle (`+ `/`+2`/`+3`; the sweep canary moved OFF the console in the logging facelift: grep `livingweapon.log` for `[display] memory sweep number 1 finished` -- ALL sweep generations are Debug-tier/file-only now). **[BLOCKER]**
