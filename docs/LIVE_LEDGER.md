...
| Death/crystal "3 hearts" counter = combat-slot `+0x07` | a KO'd unit's slot byte at base `+0x07` steps 3→2→1→0 in sync with the on-screen hearts (once per the dead unit's turn). The doc had this "unmapped in every public IC source". | `probes/crystal_counter_probe.py watch` diff; found 2026-06-16. PIN-gating test (`pin`) pending → flips PROVEN if holding it ≥1 stops crystallization | 2026-06-16 |
