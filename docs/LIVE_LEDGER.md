...
and LW-368/371/372 pass re-crosses this gate. PROVEN 2026-08-31 (owner flip). Evidence: LW351_plan.md round
...
listed, markers intact, no empty rows. PROVEN 2026-08-31 (owner flip). Evidence: LW351_plan.md rounds 8/8b,
...
PROVEN 2026-08-31 (owner flip). Evidence: LW351_plan.md round 7 (R7-1), tools/probes/lw351_bag_watch.py,
...
save fields stay the staging area, TemplateSync projecting/adopting at the edges). PROVEN 2026-08-31 (owner flip). Evidence: LW351_plan.md re-test 5 + fix round 5, the Offsets.cs template constants'
...
check_extended_range enforces it. PROVEN 2026-08-31 (owner flip, the observation half; the mechanism question stays LW-364's). Evidence:
...
rows, not nxd sort cells). PROVEN 2026-08-31 (owner flip). Evidence: LW351_plan.md fix round 1 + Y6/Z2,
tools/patch_names.py --selftest.

### [items-tab-draws-past-149] The Items tab's own drawing code copes far past the 149-entry list cap: with the two cap bytes raised to 255/256 it built and drew a 235-row weapons list with no clipping and a clean Sort

Observed 2026-08-31 10:47 by the owner on the deployed LW-371 prod build (the 121-item stunt table, a fresh save): the probe tools/probes/lw372_list_cap_probe.py raised the list builder's entry cap byte 0x140288CC3 from the shipped 0x95 (149) to 0xFF (255) and the list insert bound imm32 at 0x14028631A from 0x96 (150) to 0x100 (256), then wrote one copy of every unowned vanilla weapon and shield kind (102) onto the relocated count page. On the next Items-tab open the game's housekeeper grew the relocated chart to 251 kinds and the builder emitted a 235-entry weapons list into the static 256-word buffer 0x141811470 (the probe's watch read both), and the owner scrolled the full list and sorted it: no clipping, no garbling, no crash. The equip picker and the shops were deliberately NOT opened (their 152-word stack buffers are exactly what LW-372's hook exists to protect); the caps were restored to 0x95/0x96 after the run. PROVEN 2026-08-31 (owner flip, ~13:05): built on the same day by LW-372 and re-crossed through the SHIPPED build's own caps in the owner's live pass (161-row shop list, 153-row All Items view, both counted by the owner and matched word-for-word by a live read of the list buffer; zero warnings).

**Date:** 2026-08-31

### [list-builder-buffer-swap] The shared menu list builder can be hooked to hand its two stack-buffer callers a mod-owned buffer with a truncated copy-back, and the two list caps then widen to 255 with every menu safe

PROVEN 2026-08-31 (owner flip, ~13:05, after the live pass 12:44-12:59 on the deployed prod build). Mechanism: a detour on the list builder 0x140288B94 (16-byte prologue landmark); a call whose output buffer is NOT the static 256-word list 0x141811470 runs the original against one of four pool slots on a mod page and receives the first 149 entries plus the terminator back (149 = the LW-371 cookie bound minus the picker's two hand-item appends); the static callers pass through untouched; the caps 0x140288CC3/0x14028631A widen 149/150 to 255/256 in the same one-transaction arm. Every failure path returns 0 with an empty terminator, never an untruncated count. Live pass: armed line "5 hooks (menu list builder hooked)"; ~235-row Items tab; equip picker with both hands full on two units (no crash); a 161-row Dorter shop list and a 153-row All Items view (the first lists past the old 149 ever drawn); save/reload and a battle Retry holding every count; zero warnings all session. Suite 3587; plan LW372_plan.md v1.3 (untracked); probe tools/probes/lw372_list_cap_probe.py (its 0x95/0x96 baseline is retired by this ship). Three verification rounds; the second found the untruncated-count failure path before it ever ran.

**Date:** 2026-08-31

### [order-templates-relocatable] The two weapon menu order templates (inventory 0x1407B2550, picker 0x141874540) can be moved to a page the mod owns by re-pointing three pointer-table slots, and the game then inserts, deletes, lists, sorts and survives a save and reload with more than 140 kinds in them

Observed 2026-08-31 06:41-06:44 by the owner on 1.5.2 (give-all save, key pt2022, 141 hand-slot kinds owned), in a running game: with both templates copied onto a page at 0x13FF00000 (inventory +0, picker +0x400, marker placed at word 141, the spare room walled with 0xFFFF words) and the three slots 0x14067F498[0], 0x140689C38[0], 0x14067FA90[0] rewritten from outside (tools/probes/lw371_order_template_relocate.py --apply --caps --assume-full), the page markers stepped 141, 142, 143 on two purchases (Sunderer, Sasori) and 143 down to 133 on ten sales, the old blocks did not change by a byte across the whole run, a save to a spare slot and a reload (06:42:50 / 06:42:58) left the page at 143 and the menus listing every kind, and nothing crashed. The two menu-list caps (0x140288CC3 0x91 to 0x9F, 0x14028631A 0x92 to 0xA0) were patched alongside but NOT exercised: the largest single list built was 127 weapons (143 kinds minus 16 shields), under the vanilla 145. The build does NOT ship those bytes: the plan review found the two small list buffers hold 152 words before the stack cookie (not 160), and the second verification found that the picker's weapons-only caller (0x1402875A4) appends the unit's two hand items after the capped list before its terminator, so the buffers must hold cap + 3 words; the largest safe cap is 149 (0x95 / 0x96), which the DLL ships, proven by the frame arithmetic on disk (re-derived by the orchestrator 11:05), not seen live: a list past 145 needs 146+ hand kinds owned, which the fifteen-item table cannot reach. PROVEN 2026-08-31 (owner flip, 10:05, after the live pass below).

Two things the same session proved about the vanilla layout, both before any probe write: opening a menu with 141 owned kinds made the game's own housekeeper push the marker past word 140 on BOTH templates (06:30:37 and again 06:40:20 after a fresh load: the rebuild hook read "no 00FF end marker in its first 141 words"), the picker's marker landing on the helmet template's first word (that template then rebuilt itself in insert order), and the autosave at 06:30:48 (autoenhanced.png) plus the spare-slot saves written 06:41:56 to 06:43:28 carry the marker-less 141-id field. And the first probe version crashed the game (06:31, a Ravager purchase) by copying a marker-less template onto a page followed by zeros: the housekeeper's walk stops only at the marker or a word at or above 0x114, so it walked off the 64 KB page. The wall of 0xFFFF words and the marker-less refusal are the two rules bought with that crash.

Built on 2026-08-31 (LW-371, LivingWeapon/Extended/TemplateRelocation.cs + .Sites.cs + TemplateSync.cs, deployed prod 09:36): the mod re-points the three pointer slots and the five all-items fields at boot, walls every page chart with 0xFFFF words, writes the first 140 chart words back into the save's field before each save and copies the field onto the page after each load, and widens the two list caps to 149 (proven by frame arithmetic on disk, see the cap note above; not crossed live). The owner's eight-step pass 09:37 to 09:55 held on both launches with zero warnings: 143 kinds listed in both menus, Sort clean, seven saves and two reloads keeping every count, a sold kind vanishing, a mod-less launch loading the same save with the new kinds simply absent, a new game starting with only its seeded gear, and a lost battle plus Retry leaving the lists intact (three flight tapes). The old blocks stayed marker-less from the morning's overflow throughout, unread. The residual is the list cap past 145 (never built on this table; LW-372 replaces it).

<details><summary>How we got here</summary>

**Claim (original wording):** the 140-kinds-at-once ceiling is the templates' fixed 141-word save field; the live readers all go through pointer-table slots (finding 1 of LW371_plan.md), every direct `lea` naming a template is a whole-save-struct copy, so re-pointing the three slots moves the working templates while the save file keeps its field.

**Mechanism:** read-only sweep of the running game (the LW-368 classifier over both template spans plus a data-section qword sweep: 15 plain-code leas, all bulk copies; 3 pointer slots; the copy-protected new-game initializer at 0x150C9FB15 / 0x150C9FCAD..CC9 left alone), the three housekeepers, the rebuild, the list builder and both stack-buffer callers disassembled from the live process; VirtualProtectEx + WriteProcessMemory for the slots and the two cap bytes with read-back; a 64 KB page below the image; a 500 ms watch of both page markers, both old blocks, the all-items sub-table at picker+0x1E6 and the list buffer 0x141811470.

**Evidence:** tools/probes/lw371_order_template_relocate.py and lw371_order_template_relocate_undo.json.done (run 2) plus lw371_order_template_relocate_undo_crash1.json.crashed (run 1); the watch trace and both session logs rescued to the session scratchpad (lw371_watch.txt, lw371_crash1_livingweapon.log, lw371_run2_livingweapon.log); LW371_plan.md findings 1-10; docs/TODO.md LW-371 row.
...
Proven, owner-flipped 2026-08-30: the re-test on the masked-key build passed end to end (three loads in a row placed each save's own recorded counts under the exact keys their saves wrote, and the counts followed their slots). Built 2026-08-27 late from a static read (docs/research/ITEM_CAP_261_BREAK_JOURNEY.md "save edges"); what the read established: the save struct's pointer lives in 0x140D407A0 and normally holds the static image buffer 0x142C81C80 (the struct is NOT transient), its +0x100..+0x1B8 header carries the play time (+0x1B4) and the slot-list metadata, every save reaches the serializer 0x140218F78 through the save wrapper 0x14021B070's tail jump, the real load-apply is 0x14021B0E8 and the second restore routine is 0x14021DE98, and 0x14021DDF0 is the async file-op stepper (it runs for saves as well as loads) and is deliberately not hooked; the three hooked entries are plain entries behind clean prologues. Corrected 2026-08-27 late night: the first build read 0x141D407A0 (one digit off) and hooked the save wrapper and the stepper; the owner's live test 2 showed no edge firing, a live xref sweep found zero references to the wrong address and 46 to the right one. What the live pass must show: the serializer hook fires on a manual save AND on the autosave (both write the same struct), the apply hook fires on a load with the struct pointer still valid after the original returns (the header is read AFTER the original), the header bytes read at save time equal those read at load time for the same save (the key matches, the replay picks the right entry), and the second restore routine 0x14021DE98 does not fire with a stale struct on some unrelated action (a false replay). The null-pointer check in the detours is no longer a "was this a load?" discriminator: ten plain-code writers store the global and none of them stores zero, so every call of either hooked routine takes a load edge, which is sound because both routines overwrite the game's bag from the struct after zeroing count[0] (the load-apply's loop-exit store at 0x14021B1C8, then its copy at 0x14021B1D5; the second restore's `mov byte [rcx],0`, then its copy at 0x14021E1D1), and the live pass watches for an "A save was loaded" line on a non-load action, which would mean a routine ran on a stale struct. That widening also reaches the one-shot legacy migration (ExtendedBagSidecar.TakeLegacy, spent on the first load whose key the sidecar has never seen, persisted at once and recorded under no key of its own), which any spurious edge can now spend on a load the owner never made; today legacy equals the seed ({261: 1}) so nothing can be lost, and the live pass checks afterwards that extended_inventory.json's `saves` map holds only keys the owner actually saved or loaded and no `legacy` key. Update 2026-08-28 after the owner's live run: the hooks themselves did their job (the load-apply canary fired, five saves were recorded and the migration was placed; the second restore routine 0x14021DE98 never fired in that run, so which game action drives it is still unanswered), but no save recognized its own load, so every load still fell back to the first-copy seed. The cause is three bytes inside the hashed window: offsets 0x1A, 0x1C and 0x1D of it (header +0x11A, +0x11C, +0x11D) read 0xFF at the instant a save edge samples, right after the serializer returns, and 0x00 at rest, which is the state every observed load sampled, so the same save hashed two ways. The key now zeroes those three bytes first (Offsets.SaveHeaderVolatileOffs), in the mod and in both probes; reconstructing the run's headers offline reproduced all five of that night's logged save keys from the resting headers the round-trip probe captured (tools/probes/lw353_header_roundtrip.py) and made save key equal load key for every observed pair. The five entries that run wrote keep their unmasked names and are orphans under the mask (harmless under the 64-key cap), and the one-shot legacy migration was consumed in the same run, placed into the bag once and then cleared, recorded under no key at all (its payload equaled the seed), so the re-test judges the `saves` map on the keys written after the mask. OWNER FLIP 2026-08-30 (re-test 15:18-15:24): load pt2684 placed x3 and loads pt2881 and pt2872 each placed x2, every one "from its own recorded counts" under the exact key its save wrote; the first-copy seed and the already-spent migration behaved as designed; the sidecar held only keys the owner actually wrote; no loaded line fired on a non-load action. The second restore routine 0x14021DE98 has still never fired; which game action drives it stays an open observation, watched by its own named canary.
...
**Evidence (correction):** tools/probes/lw346_xref_scan.py (46 references to 0x140D407A0, none to 0x141D407A0); the disasm at 0x14021B101 (the load-apply reading the global as `4C 8B 05 98 56 B2 00`), at 0x14021B0E3 (the save wrapper's tail jmp into the serializer, which puts the apply's entry at the next byte 0x14021B0E8) and at 0x14021DE98 (the second restore's prologue, with its bag copy at 0x14021E1D1); and 2026-08-27's livingweapon.log, which shows all three hooks installed and not one canary line.

**Evidence (mask):** tools/probes/lw353_header_roundtrip.py's two live captures of a resting header (2026-08-28 01:09:27 and 01:09:53); the five save keys the mod's own log recorded that night (pt2200-f0fefc093c1b, pt2212-7423df91f46f, pt2219-0dbabc353233, pt2269-fb60d7fd7608, pt2584-f25ab97abe23), each reproduced offline from the matching resting header by setting the three bytes to 0xFF, and the run's four load keys (pt2147-16f352195175, pt2269-b0d816f46ff3, pt2219-4ee338125549, pt2584-ac68fc314b36), every one of them a resting-form key; and LivingWeapon.Tests SaveEdgeTrackerTests.Key_masks_the_save_transient_bytes, which pins both states of the captured pt2219 header to one key.

**Date:** 2026-08-27; owner flip 2026-08-30
