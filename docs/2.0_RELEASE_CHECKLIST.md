...
- [x] **Equip-card kills counter + suffix paint** out of battle (`+ `/`+2`/`+3`, `display: memory sweep #1 finished -- maintaining M card-text spots` on the console; only generation #1 is Info, later re-sweeps are Debug-tier/VerboseLog-gated). **[BLOCKER]**
