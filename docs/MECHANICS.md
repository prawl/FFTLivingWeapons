...
> LEDGER STATUS (LW-107, 2026-07-21): these rows sat in LIVE_LEDGER's **Uncertain** section for
> eleven days, each tagged "Ready for the PROVEN flip" that nobody performed, while the write-ups
> here called them proven. The audit surfaced the gap and the OWNER FLIPPED THEM: teleport/swap/
> float, spawn, despawn, resurrect, and hide/reveal are now **Proven** rows, so the labels below
...
- SPAWN A REAL AI FIGHTER MID-BATTLE (LW-58 Body Double, COMPLETE) -- PROVEN LIVE 2026-07-10:
  ...
  live-proven): copy the donor's combat struct into a vacant same-region slot at a FREE tile; clone the
...
- Dual-wield double credit: if a Living Weapon wielder fights with a weapon in EACH hand, one kill credits BOTH weapons at once, not just one -- SHIPPED/deliberate. Rationale: a dual-wield Attack really does strike with both blades, and the death event carries no record of which hand landed the finishing blow, so crediting only one would be fake precision, not a true read of what happened. Edge cases: a shield never earns credit, in either hand; an off-hand weapon still counts even with a shield in the primary hand (the item decides, not the slot); the same weapon id equipped in both hands counts once, not twice. Test-pinned (LivingWeapon.Tests/KillTrackerTests.cs): Credits_both_weapons_of_a_dual_wielder, Does_not_credit_a_shield_in_the_off_hand, Credits_an_off_hand_weapon_even_with_a_shield_in_the_primary, Counts_the_same_weapon_in_both_hands_only_once. Practical consequence: dual-wielding two Living Weapons is the single fastest way to level a pair of blades, since every kill grows both tallies at once. (LW-272, owner-reported 2026-08-18.)
