...
| ...or you REMOVE Living Weapons while a save holds one of its brand-new weapons (the extended inventory, ids past the game's own 261: today the Moonblade and the Terrastaff) | The game only knows those items while the mod is running. Without it the save still loads fine: the unit's hand is simply emptied and the item is gone from the bag, nothing crashes. Best practice before uninstalling: unequip or sell the new weapons first. If an inventory list then looks short (an entry seems to be missing after the spot where the new weapon sat), sort that list once and it repairs itself | Everything else in the save; the new weapon comes back, with its saved bag count, the moment the mod is reinstalled |
