...
Throwaway conveniences for testing this mod live. None of this ships â€” it's harness only.
...
â†’ `give_all_items [count]`. Don't reinvent it.
...
- Requires the **FFTHandsFree mod loaded** in the running game (it drives the `fft` commandâ†’`command.json`
  bridge). Both `FFTHandsFree` and `prawl.fft.itemoverhaul` can be loaded at once.
- **Run on a safe screen** (WorldMap / TravelList / battle). NOT while PartyMenu is open â€” roster-adjacent
  writes get clobbered. Close + reopen the inventory afterward to refresh the menu.

**The underlying recipe** (works via plain RPM/WPM too, no bridge needed â€” see `give_living_blades.py`):
...
**Skip the crashy / IC-unused ids:** `262` (Onion Sword crashes on equip-render) and `261, 263â€“277`
(IC stripped these slots). `give_all_items` already skips them; replicate the skip if you write your own.
Caveat: only ids whose IC layout matches FFTPatcher-canonical render in the menu (~80â€“85%); the rest write
into RAM but don't show.

## Bump a weapon's WP for one-hit-kill testing

Edit `data/items.json` â†’ the item's `proposed.wp` (NOT `baseline`) â†’ `BuildLinked.ps1` â†’ restart the game
...
`%TEMP%\fft_probes\ct_probe.py` â€” RPM/WPM (can't crash the game) watch/dump/hold of the battle structs. Modes:
