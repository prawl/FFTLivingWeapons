...
internal sealed partial class GrowthEngine
{
    ...
    // the LW-90 NaturalLedger (decision 3, pinned by HoldU16_RestartResidue_DecisionPin):
    ...
    // swords. Consequence, ACCEPTED: across a battle-RESTART chain the held MaxHp can be
    // re-captured as natural and creep upward -- clamped at Tuning.HpCeiling every time,
    // battle-scoped (this dict is cleared by ResetBattle like every other applied-lane dict),
    // and fully self-corrected the moment a fresh roster load recaptures the unit's true stats.
    ...
    /// Apply, but this lane never reads them). Re-applies on cur == natural (the engine's
    /// per-turn normalize); retargets in place when factor changes at cur == target (a kill-tier
    /// crossed mid-battle); leaves any other reading alone (a real HP-affecting effect this
    /// runtime must never fight).</summary>
    internal void HoldU16(long addr, double factor, int ceiling, StatLane lane, int nameId, int level)
    {
        ...
        {
            if (cur == e.target) { if (factor != e.factor) WriteU16Target(addr, e.natural, factor, ceiling); return; }
            if (cur == e.natural) WriteU16Target(addr, e.natural, factor, ceiling);
            return;   // anything else: leave it
        }
        if (cur >= U16CaptureMin && cur <= U16CaptureMax)
            WriteU16Target(addr, cur, factor, ceiling);
    }

    private void WriteU16Target(long addr, int natural, double factor, int ceiling)
    {
