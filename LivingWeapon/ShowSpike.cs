...
internal sealed class ShowSpike
{
    ...
    public void Arm(IReloadedHooks hooks)
    {
        ...
        try
        {
            ...
            Log.Info("show-spike: banner-update + orchestrator + text-setter tap hooks installed -- canaries are the ALIVE lines (a natural callout bubble with no 'orch CALLED' line = dead launch, restart)");
        }
        catch (Exception ex)
        {
            Log.Error("show-spike: hook install failed -- " + ex.Message);
        }
    }
    ...
    public void Tick()
    {
        ...
        {
            Log.Info($"show-spike: alive (hooks={(_updateHook != null ? "yes" : "NO")}/{(_orchHook != null ? "yes" : "NO")} canaries={_updateCanary}/{_orchCanary} captures={_haveLast}/{_haveName} chase={_chase})");
        }
        ...
        {
            _promptSwap = true;
            Log.Info($"show-spike: F6 -- PROMPT SWAP armed (one-shot): the next facing prompt renders \"{PromptSwapText}\" and should STAY until you confirm facing");
        }
        if (Pressed(VkF5, ref _f5Was))
        {
            ...
            Log.Info($"show-spike: F5 -- COMMIT TAP armed {TapWindowSeconds}s, deduped (end an action and sit in the facing prompt; hunting its text writer)");
        }
    ...
    private void Queue(FireMode mode, string what)
    {
        if (_updateHook == null)
        {
            Log.Info("show-spike: key pressed but no hooks installed (IReloadedHooks never arrived)");
            return;
        }
        if (mode == FireMode.ArmChase)
        {
            _chase = true;
            Log.Info($"show-spike: {what} -- CHASE armed: id-patched replay fires right after the NEXT natural callout returns (cast something; watch for the WRONG string in the bubble)");
            return;
        }
        ...
        _mutate = true;
        Log.Info($"show-spike: {what} -- MUTATE armed: the NEXT natural callout's own token id gets patched in-flight (cast something; the natural bubble should show the WRONG string)");
    }
    ...
    private void SnapshotAfter(long ctrl, uint typeAtEntry)
    {
        ...
        string ws = w != 0
            ...
            : "widget=NULL";
        Log.Info($"show-spike: post-ritual: {ws} obj={obj:x} obj218={(obj != 0 ? _mem.U32(obj + 0x218) : 0)} obj280={(obj != 0 ? _mem.U8(obj + 0x280) : 0):x2} holder88={_mem.U8(holder + 0x88)}");
    }
    ...
    private void OrchDetour(nint ctrl, nint textObj, nint hasText, nint r9)
    {
        ...
        {
            _orchCanary = true;
            Log.Info($"show-spike: orch hook ALIVE (thread {GetCurrentThreadId()})");
        }
        uint type = 0;
        try
        {
                ...
                if (!_firing)
                {
                    ...
                    if (_mutate)
                    {
                        ...
                        if (FlipId(patched))
                        {
                            ...
                            Log.Info($"show-spike: MUTATE -- patched the natural request's id in-flight ({bytes[IdOffset + 1]:x2}{bytes[IdOffset]:x2} -> {patched[IdOffset + 1]:x2}{patched[IdOffset]:x2}); does the bubble show the WRONG string?");
                        }
                        else
                        {
                            Log.Info("show-spike: MUTATE skipped -- unrecognized id in the natural stream");
                        }
                    }
                }
            }
            ...
            if (!_firing && _stackCaps > 0)
            {
                ...
                Log.Info($"show-spike: STACK type={type} thread={GetCurrentThreadId()} frames[{n}]: {sb.ToString().TrimEnd()}");
                ...
                unsafe
                {
                    ...
                    Log.Info($"show-spike: STACKSCRAPE type={type} span=0x{span:x} hits[{hits}]: {hitSb.ToString().TrimEnd()}");
                }
            }
            Log.Info($"show-spike: orch CALLED{(_firing ? " (OUR replay)" : "")} thread={GetCurrentThreadId()}: ctrl={(long)ctrl:x} type={type} textObj={(long)textObj:x} r8={(long)hasText:x} tokens={tok}");
        }
        catch (Exception ex)
        {
            Log.Error("show-spike: orch detour log failed -- " + ex.Message);
        }
        ...
        try
        {
            ...
            {
                _chase = false;
                Log.Info("show-spike: CHASE firing -- ritual + custom-text commit in the natural call's own thread/context");
                Fire((long)ctrl, patchId: true);
            }
        }
        catch (Exception ex)
        {
            Log.Error("show-spike: orch detour post failed -- " + ex.Message);
        }
    }
    ...
    private void TapDetour(IHook<TextCommitTapFn> hook, string which, nint holder, nint text, nint r8, nint r9)
    {
        try
        {
            ...
            {
                _tapCanary = true;
                Log.Info($"show-spike: commit-tap hooks ALIVE (first {which}, thread {GetCurrentThreadId()})");
            }
            ...
            if (tapOpen || swapEligible)
            {
                ...
                if (tapOpen)
                {
                    ...
                    if (fresh && System.Threading.Interlocked.Increment(ref _tapLines) <= TapMaxLines)
                        Log.Info($"commit-tap: {which} holder=0x{(long)holder:X} text=0x{(long)text:X} \"{preview}\" (tid {GetCurrentThreadId()})");
                }
                if (swapEligible && preview.StartsWith(FacingPromptPrefix, StringComparison.Ordinal))
                {
                    ...
                    Log.Info($"show-spike: PROMPT SWAP fired -- holder=0x{(long)holder:X} now carries \"{PromptSwapText}\" (does it render and STAY until confirm?)");
                }
            }
        }
        catch (Exception ex)
        {
            Log.Error("show-spike: commit-tap log failed -- " + ex.Message);
        }
    ...
    private void UpdateDetour(nint bannerObj, float dt)
    {
        ...
        {
            _updateCanary = true;
            Log.Info($"show-spike: update hook ALIVE (thread {GetCurrentThreadId()})");
        }
    }
    ...
    private void Fire(long ctrl, bool patchId = false)
    {
        if (!_haveLast)
        {
            Log.Info("show-spike: no capture yet this launch -- cast something first, then retry");
            return;
        }
    ...
    private void FireStream(long ctrl, byte[] src, uint type, bool patchId, string mode, nint text, string textLabel)
    {
        try
        {
            ...
            _firing = false;
            Log.Info($"show-spike: wrapper fired (mode={mode}{patched} ctrl={ctrl:x} type={type} thread={GetCurrentThreadId()})");
            ...
            Log.Info($"show-spike: custom text committed after the ritual -- does the bubble say '{textLabel}'?");
        }
        catch (Exception ex)
        {
            _firing = false;
            Log.Error("show-spike: managed exception in game-thread fire -- " + ex.Message);
        }
    }
    ...
    private void ColdSpawn()
    {
        ...
        if (ctrl == 0)
        {
            Log.Info("show-spike: F4 -- controller chain unresolved (enter a battle first)");
            return;
        }
        if (!_haveName)
        {
            Log.Info("show-spike: F4 -- no type-0 capture yet this launch (do one attack first, then retry)");
            return;
        }
        try
        {
            uint timer = _mem.U32(ctrl + LingerOffset);
            Log.Info($"show-spike: F4 COLD SPAWN (loop thread {GetCurrentThreadId()}) -- pre: flag={_mem.U8(ctrl + ShowFlagOffset)} timer={timer}");
            ...
            helper((nint)ctrl, -2);
            helper((nint)ctrl, 0);
            Log.Info($"show-spike: F4 sequence complete -- post: flag={_mem.U8(ctrl + ShowFlagOffset)} timer={_mem.U32(ctrl + LingerOffset)} -- IS THERE A BUBBLE?");
        }
        catch (Exception ex)
        {
            Log.Error("show-spike: F4 cold spawn failed -- " + ex.Message);
        }
    }
    ...
    private void ColdDismiss()
    {
        ...
        if (ctrl == 0)
        {
            Log.Info("show-spike: F6 -- controller chain unresolved (enter a battle first)");
            return;
        }
        try
        {
            ...
            var helper = Marshal.GetDelegateForFunctionPointer<HelperFn>(unchecked((nint)FnStreamHelper));
            helper((nint)ctrl, -2);
            Log.Info($"show-spike: F6 DISMISS fired -- pre flag={flag} timer={timer}, post flag={_mem.U8(ctrl + ShowFlagOffset)} -- did the bubble vanish?");
        }
        catch (Exception ex)
        {
            Log.Error("show-spike: F6 dismiss failed -- " + ex.Message);
        }
    }
}
#endif
