...
/// WeaponPalette.Policy.cs's WeaponPalettePolicy.Decide for the full seven-row decision table this
/// class only executes; this file is the stateful (memory-touching) half. LW-295's "what glow
/// tier do we want" reads live in the WeaponPalette.Desire.cs partial (a separate seam: this file
/// is "how the banks are painted", Desire.cs is "what do we want painted").
///
...
internal sealed partial class WeaponPalette
{
    ...
    /// <summary>LW-295: the shared kill tally (Engine._kills), held by reference like every other
    /// subsystem -- WeaponPalette.Desire.cs's DesiredGlow keys the acting wielder's glow tier off
    /// it.</summary>
    ...
    private int _paintedGlow;
    private int _ticksUnchanged;

    public WeaponPalette(Dictionary<int, WeaponMeta> meta, IGameMemory mem, Dictionary<int, int> kills)
    {
        _meta = meta;
        _mem = mem;
        _kills = kills;
    ...
    public void Tick(bool inLive)
    {
        ...
        int desiredPal = -1, desiredWeapon = -1, desiredGlow = 0;
        ...
        if (_meta.TryGetValue(wid, out var m) && m.Colors is { Length: 15 } && m.Palette is >= 0 and <= 15)
        {
            ...
            desiredWeapon = wid;
            desiredMeta = m;
            desiredGlow = DesiredGlow(_mem, entry, wid);
        }

        var action = WeaponPalettePolicy.Decide(_paintedPal, _paintedWeapon, _paintedGlow,
            desiredPal, desiredWeapon, desiredGlow, _ticksUnchanged, ReassertTicks);
        ...
        switch (action)
        {
            ...
            case WeaponPaletteAction.Restore:
                ...
                _paintedPal = -1; _paintedWeapon = -1; _paintedGlow = 0; _ticksUnchanged = 0;
                break;
            case WeaponPaletteAction.Paint:
                PaintBanks(desiredPal, desiredWeapon,
                    WeaponPalettePolicy.BrightenAll(desiredMeta!.Colors!, Tuning.GlowFactors[desiredGlow]));
                _paintedPal = desiredPal; _paintedWeapon = desiredWeapon; _paintedGlow = desiredGlow; _ticksUnchanged = 0;
                break;
    ...
    /// snapshot (the other bank still paints). Must never mutate <paramref name="codes"/> itself
    /// -- at tier 0 BrightenAll hands this the shared session-lifetime meta Colors array by
    /// reference, so a mutation here would corrupt the baked meta for the rest of the
    /// session.</summary>
